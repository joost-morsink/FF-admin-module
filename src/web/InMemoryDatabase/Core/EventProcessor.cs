    ...
    where T : class
{
    ...
    TypedDictionary IApplyToTypedDictionary.Process(TypedDictionary result,
        IHistoricContext historicContext, Event e)
        => result.Set(() => Process(historicContext.Previous.GetContext<T>()!, historicContext, e));
}
